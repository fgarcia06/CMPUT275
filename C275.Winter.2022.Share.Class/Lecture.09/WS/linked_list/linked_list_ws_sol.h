/*
  ...
  Includes the worksheet solutions.
...
public:
  ...
  // Find and return a pointer to the last node with the item.
  // Returns the NULL pointer if the item is not in the list.
  ListNode<T>* findLast(const T& item) const;

  // Removes all but the first occurrence of the item in the list.
  // If the item is not in the list, do nothing.
  void removeAllButOne(const T& item);

  // Reverse this entire linked list.
  void reverse();
