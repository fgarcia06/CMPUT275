...
public:
  ...
  // the default tableSize of the HashTable is 10
...
/*
  Description: Inserts an item into the hashtable via one of the buckets. If the number of items get larger than the bucket count (congested), double the size dynamicarray. 

  Arguments: item (const value, passed by reference with template type T; the item you want to put into the hash table)

  Returns: bool true or false, false if the item is already present in the hashmap, true if not, and successfully inserts it in
*/
template <typename T>
bool HashTable<T>::insert(const T& item) {
  ...
  if (contains(item)) {
    ...
    if (numItems > getBucketCount()){
      DynamicArray<T> array = getItemsArray(); //get the old hashmap
      tableSize = 2*tableSize; //double table size
      table = new LinkedList<T>[tableSize]; //dynamically allocate memory from the heap to making a new array containing linked lists of size tableSize

      for (unsigned int i = 0; i < array.size(); i++){
        bucket = getBucket(array[i]); //find the bucket of the specific item
        table[bucket].insertFront(array[i]); //insert it into the bucket
      }
    }
    return true;
  }
...
/*
  Description: Removes an item from hashtable; making sure the item is there first, and if the number of items drop to a fourth of the bucket count (while the bucketCount is > 10), 
    to resize the hashtable.

  Arguments: item (cnst value, passed by reference with template type T; the item you want to remove from the hast table).
...
void HashTable<T>::remove(const T& item) {
  ...
  if ((numItems < (getBucketCount()/4)) && (getBucketCount() > 10)){
    DynamicArray<T> array = getItemsArray();

    tableSize = max(int(getBucketCount())/2, 10);
    table = new LinkedList<T>[tableSize];

    for (unsigned int i = 0; i < array.size(); i++) {
      bucket = getBucket(array[i]);
      table[bucket].insertFront(array[i]);
    }
  }
}
